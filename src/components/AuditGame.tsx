...
interface AuditGameProps {
  onComplete: (xp: number, score: number, gameName: string, duration?: number) => void;
  onExit: () => void;
...
const AuditGame: React.FC<AuditGameProps> = ({ onComplete, onExit, isRealWorldStart = false, checklist = [], initialContext = '' }) => {
  ...
  const startRef = useRef<number>(Date.now()); // Track start time

  // Reset timer when entering a specific mode
  useEffect(() => {
    if (selectedScene || isLensMode) {
      startRef.current = Date.now();
    }
  }, [selectedScene, isLensMode]);
  ...
  const syncTag = async (index: number) => {
    ...
    try {
      ...
      if (newHazards.length === 0) {
        ...
        onComplete(150, 100, "AR 5S Audit", duration);
      }
  ...
  if (selectedScene) {
    return (
      <div className="max-w-4xl mx-auto space-y-8 animate-fade-in pb-32">
        ...
        <button onClick={() => {
          const duration = (Date.now() - startRef.current) / 1000;
          onComplete(selectedScene.xpReward, 100, selectedScene.title, duration);
        }} className="w-full py-6 bg-red-600 text-white rounded-3xl font-black uppercase tracking-widest shadow-2xl flex items-center justify-center group">{t('common.finish')} <ChevronRight className="w-6 h-6 ml-4" /></button>
      </div>
    );
  }
  return null;
