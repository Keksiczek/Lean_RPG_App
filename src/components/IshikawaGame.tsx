...
import { generateSolutions, generateIshikawaProblem } from '../services/geminiService';
import { gameService } from '../services/gameService';
import { useAuth } from '../contexts/AuthContext';
import { ArrowLeft, Plus, Trash2, BrainCircuit, Loader2, CheckCircle2, ChevronRight, PenTool, Sparkles } from 'lucide-react';
...
const IshikawaGame: React.FC<IshikawaGameProps> = ({ onComplete, onExit, isRealWorldStart = false }) => {
  ...
  const handleCreateDailyChallenge = async () => {
    ...
    try {
      const newProblem = await generateIshikawaProblem();
      setSelectedProblem(newProblem);
      setPhase('analysis');
    } catch (e) {
      alert("Failed to create daily challenge. Check API Key.");
    } finally {
    ...
    return (
      <div className="space-y-6 animate-fade-in">
        ...
        <div className="flex justify-between items-center mb-6">
          ...
          <button
            onClick={handleCreateDailyChallenge}
            disabled={isCreatingProblem}
            className="bg-gradient-to-r from-amber-500 to-orange-600 text-white px-5 py-2 rounded-full font-bold shadow-lg hover:shadow-xl hover:scale-105 transition-all flex items-center"
          >
            {isCreatingProblem ? <Loader2 className="w-4 h-4 mr-2 animate-spin" /> : <Sparkles className="w-4 h-4 mr-2" />}
