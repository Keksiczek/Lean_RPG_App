...
import { Trophy, Medal, Crown, Filter, ArrowUpDown } from 'lucide-react';
import { useAuth } from '../contexts/AuthContext';
import { DEMO_LEADERBOARD } from '../constants';
...
const Leaderboard: React.FC<LeaderboardProps> = ({ initialSkill }) => {
  ...
  // Use demo data with current user injected
  const rankings = useMemo(() => {
    const baseData = [...DEMO_LEADERBOARD] as LeaderboardEntry[];

    // Inject logged-in user into the list if they exist
    if (user) {
      ...
      if (!baseData.find(e => e.userId === user.id)) {
        baseData.push(userEntry);
      }
    }

    // Sort and recalculate ranks
    const sorted = baseData.sort((a, b) => sortBy === 'xp' ? b.xp - a.xp : b.level - a.level);
        ...
        <div className="divide-y divide-slate-100 dark:divide-slate-800">
          {rankings?.map((entry) => {
            ...
            return (
              <div
                key={entry.userId}
                className={`flex items-center p-4 transition-colors ${isMe ? 'bg-red-50 dark:bg-red-900/10 ring-1 ring-inset ring-red-100 dark:ring-red-900/20' : 'hover:bg-slate-50 dark:hover:bg-slate-800'
              ...
              >
                ...
                <div className="flex-1 flex items-center">
                  <div className={`w-10 h-10 rounded-full flex items-center justify-center mr-3 font-bold shadow-inner ${entry.rank === 1 ? 'bg-yellow-200 dark:bg-yellow-900/50 text-yellow-800 dark:text-yellow-400' :
