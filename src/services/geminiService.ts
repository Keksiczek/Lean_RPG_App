...
import { IshikawaCause, IshikawaSolution, LensScanResult, LPAScanResult, IshikawaProblem, Difficulty } from "../types";

// Helper to get API Key securely from storage or env
const getApiKey = (): string => {
  const storedKey = localStorage.getItem('lean_rpg_api_key');
  // Fallback to env if not found in storage (for dev ease)
...
const ishikawaProblemSchema = {
  type: Type.OBJECT,
  properties: {
    title: { type: Type.STRING, description: "Short, punchy title of the manufacturing problem" },
    description: { type: Type.STRING, description: "Detailed 1-2 sentence scenario description with context" },
    category: { type: Type.STRING, description: "Primary affected area (e.g. Man, Machine, Method)" },
    difficulty: { type: Type.STRING, enum: ["easy", "medium", "hard"] }
  },
  required: ["title", "description", "category", "difficulty"]
};

export const generateIshikawaProblem = async (context?: string): Promise<IshikawaProblem> => {
  try {
    const ai = new GoogleGenAI({ apiKey: getApiKey() });

    // Day of week factor for variance
    const day = new Date().toLocaleDateString('en-US', { weekday: 'long' });

    const prompt = `
      Role: Factory Manager.
      Task: Report a new, unique, and realistic manufacturing incident for the Root Cause Analysis team.
      
      Constraints:
      - It must be a specific, credible problem (e.g., "Hydraulic press 3 overheating", "Label printer skipping serial numbers").
      - Make it sound like a real Gemba incident.
      - Vary the difficulty.
      - Current context/shift: ${context || `General Production, ${day} Shift`}.
    ...
    const response = await ai.models.generateContent({
      ...
      config: {
        ...
        responseSchema: ishikawaProblemSchema,
    ...
    return {
      ...
      isRealWorld: false // It's AI generated simulation
    };
  } catch (error: any) {
...
export const generateSolutions = async (
  ...
  try {
    ...
    const response = await ai.models.generateContent({
      model: "gemini-2.0-flash-exp", // Updated to faster model
      contents: prompt,
      config: {
        responseMimeType: "application/json",
...
export const analyze5SImage = async (base64Image: string, context?: string, checklist: string[] = []): Promise<LensScanResult> => {
  try {
    ...
    if (error.message === 'MISSING_API_KEY') {
      return { overallCompliance: 0, observation: "MISSING_API_KEY", detectedHazards: [], checklistResults: [], itemDetected: "Config Error", suggested5SAction: "Check Settings", practicalAction: "Please add API Key in Settings" };
    }
    console.error("5S Scan Error:", error);
    return { overallCompliance: 0, observation: "Scan failure", detectedHazards: [], checklistResults: [], itemDetected: "Error", suggested5SAction: "Retry", practicalAction: "Check connection" };
  }
};
...
export const analyzeLPAImage = async (base64Image: string, question: string, expectedAnswer: string): Promise<LPAScanResult> => {
  try {
    ...
    if (error.message === 'MISSING_API_KEY') {
      return { verified: false, confidence: 0, compliance: "Low", observations: ["Missing API Key"], safetyRisk: false, identifiedIssues: ["Please configure API Key in Settings"] };
    }
...
export const sendMessageToSensei = async (message: string, context?: string): Promise<string> => {
  try {
    ...
    chatSession = null;
    if (error.message === 'MISSING_API_KEY') return "Please configure your Gemini API Key in Settings to talk to the Sensei.";
